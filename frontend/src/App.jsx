...
function App() {
  ...
  useEffect(() => {
    ...
    setDadosDashboard(prev => ({ 
      ...
      total_pedidos_carteira: pedidos.length, 
            ...
            <div className={`${t.card} rounded-2xl shadow-md p-4 md:p-6 w-full border ${t.border}`}>
              <div className={`flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 border-b ${t.border} pb-5`}>
                <div>
                  ...
                  <p className={`text-xs ${t.textSecondary} mt-1`}>O Faturamento Parcial abate os valores da tela.</p>
                </div>
                ...
                <div className={`flex flex-col xl:flex-row items-center gap-3 w-full xl:w-auto`}>
                  <div className="relative w-full xl:w-64 flex-shrink-0">
                    ...
                    <input type="text" value={termoPesquisa} onChange={(e) => setTermoPesquisa(e.target.value)} placeholder="Pesquisar pedido ou cliente..." className={`w-full pl-9 pr-3 py-3 md:py-2 bg-transparent border ${t.border} rounded-lg text-xs md:text-sm ${t.textPrimary} focus:outline-none focus:border-[#5DD62C]`} />
                  </div>
                  
                  <div className="flex flex-col sm:flex-row w-full xl:w-auto justify-between items-stretch sm:items-center gap-3">
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                      <select value={ordenarPor} onChange={(e) => setOrdenarPor(e.target.value)} className={`w-full sm:w-auto ${t.card} text-xs font-bold ${t.textPrimary} border ${t.border} rounded-lg px-3 py-3 md:py-2 focus:outline-none focus:${t.borderAccent} cursor-pointer`}>
                        <option value="prazo">Dias Restantes</option><option value="id_pedido">Nº Pedido</option><option value="emissao">Data Emissão</option>
                      </select>
              ...
              {carregando ? (<div className={`p-12 text-center ${t.textSecondary}`}>Carregando dados dos pedidos...</div>) : (
                <>
                  <div className={modoExibicao === 'lista' ? 'flex flex-col gap-3' : 'grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5'}>
                    {pedidosEmAbertoPaginados.map((pedido) => {
                      ...
                      return modoExibicao === 'lista' ? (
                        <div key={pedido.id} className={`${t.inner} rounded-xl border ${t.border} ${t.hoverBorderAccent} transition-all overflow-hidden`}>
                          <div className="p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                            ...
                            <div className="flex items-start gap-4 flex-1 min-w-[240px]">
                              ...
                              <div>
                                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                                  ...
                                  <span className={`text-[11px] px-2 py-0.5 rounded-md font-mono font-bold border ${todasConcluidas ? `${t.bgAccentSoft} ${t.textAccent} ${t.borderAccentSoft}` : `${t.card} ${t.textSecondary} ${t.border}`}`}>OFs Ativas: {concluidas}/{total}</span>
                                </div>
                            ...
                            <div className={`flex flex-wrap items-center gap-4 md:gap-8 text-xs text-gray-300 w-full md:w-auto justify-between md:justify-end border-t ${t.border} md:border-t-0 pt-4 md:pt-0`}>
                              ...
                              <div className="text-left md:text-center w-[45%] md:w-auto">
                                <span className={`block text-[10px] ${t.textSecondary} uppercase tracking-wider mb-0.5`}>Peso Faltante</span>
                                <span className={`font-mono font-bold ${t.textAccent}`}>{formatarKg(pedido.peso_total_kg)} kg</span>
                              </div>
                              <div className="text-left md:text-center w-[45%] md:w-auto">
                        ...
                        <div key={pedido.id} className={`${t.inner} p-4 md:p-5 rounded-2xl border ${t.border} flex flex-col justify-between ${t.hoverBorderAccent} transition-colors`}>
                          <div>
                            <div className="flex justify-between items-start mb-3 gap-2">
                              <div>
                                <div className="flex items-center gap-2 flex-wrap mb-2">
                                  ...
                                  <span className={`text-[10px] px-2 py-0.5 rounded-md font-mono font-bold border ${todasConcluidas ? `${t.bgAccentSoft} ${t.textAccent} ${t.borderAccentSoft}` : `${t.card} ${t.textSecondary} ${t.border}`}`}>OFs Ativas: {concluidas}/{total}</span>
                                </div>
                                <h3 className={`font-bold ${t.textPrimary} text-base leading-tight`}>{pedido.cliente}</h3>
                              </div>
                              <div className="flex-shrink-0">{renderizarTagPrazo(pedido.dias_restantes, pedido.data_entrega)}</div>
                            </div>
                            <div className={`my-4 py-3 border-y ${t.border} grid grid-cols-3 gap-2 text-center text-xs`}>
                              <div><span className={`block text-[10px] ${t.textSecondary} uppercase tracking-wider mb-1`}>Itens Ativos</span><span className={`font-mono font-bold ${t.textPrimary}`}>{pedido.total_itens_abertos || 0}</span></div>
                              <div><span className={`block text-[10px] ${t.textSecondary} uppercase tracking-wider mb-1`}>Peso Faltante</span><span className={`font-mono font-bold ${t.textAccent}`}>{formatarKg(pedido.peso_total_kg)} kg</span></div>
                    ...
                    {pedidosEmAberto.length === 0 && (<div className={`col-span-full p-8 text-center ${t.textSecondary} ${t.card} rounded-2xl border ${t.border}`}>{termoPesquisa ? 'Nenhum pedido encontrado.' : 'Nenhum pedido em aberto encontrado.'}</div>)}
                  </div>
                  
                  {pedidosEmAberto.length > limiteExibicao && (
                    <div className="flex justify-center mt-6">
                      <button onClick={() => setLimiteExibicao(prev => prev + 20)} className={`px-6 py-3 rounded-xl text-sm font-bold shadow-md transition-all bg-[#5DD62C] hover:bg-[#337418] text-[#0F0F0F] hover:text-[#F8F8F8] w-full md:w-auto hover:scale-105`}>
                        Carregar Mais Pedidos ({pedidosEmAberto.length - limiteExibicao} restantes)
                      </button>
                    </div>
                  )}
                </>
